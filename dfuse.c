...
void dfuse_parse_options(const char *options)
{
	...
	while (*options) {
		...
		if (!strncmp(options, "no-erase", endword - options)) {
			dfuse_no_erase = 1;
			options += 8;
			continue;
		}
			  ...
			  enum dfuse_command command)
{
	...
	do {
		...
		if (ret < 0) {
			errx(EX_IOERR, "Error during special command \"%s\" get_status (ret:%d)",
			     dfuse_command_name[command], ret);
		}
		if (firstpoll) {
			firstpoll = 0;
      ...
      if (command == MASS_ERASE)
      {
        switch(dif->vendor)
        {
          case 0x28e9:  //gigadevice
            printf("found GigaDevice\n");
            break;

          case 0x0483:   //stm32
            printf("found STM32\n");
            if (dst.bwPollTimeout == 100) {
		...
		if (verbose)
			printf("   Poll timeout %i ms (state:%d)\n", dst.bwPollTimeout, dst.bState);
		    ...
		    const char *dfuse_options)
{
	...
	if (dfuse_address) {
		...
		mem_layout = parse_memory_layout((char *)dif->alt_name, dif->flash_sectors, dif->flash_page_size);
		if (!mem_layout)
			 ...
			 int xfer_size)
{
  ...
  if(!dfuse_no_erase)
    ...
    if (!verbose && !dfuse_no_erase)
      dfu_progress_bar("Erase   ", p, dwElementSize);

		/* Erase only for flash memory downloads */
		if ((segment->memtype & DFUSE_ERASABLE) && !dfuse_mass_erase && !dfuse_no_erase) {
		    ...
		    const char *dfuse_options)
{
	...
	if (dfuse_mass_erase && !dfuse_no_erase) {
		if (!dfuse_force) {

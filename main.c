...
int main(int argc, char **argv)
{
  ...
  uint16_t flash_page_size = 0;
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "hVvleE:d:p:c:i:a:S:t:U:D:Rs:Z:N:P:", opts,
				&option_index);
		if (c == -1)
			break;
		...
		case 'N':
			flash_sectors = parse_number("flash_sectors", optarg);
			break;
		case 'P':
			flash_page_size = parse_number("flash_page_size", optarg);
			break;
  ...
  // store user values
  dfu_root->flash_sectors = flash_sectors;
  dfu_root->flash_page_size = flash_page_size;

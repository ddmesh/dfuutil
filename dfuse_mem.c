...
struct memsegment *parse_memory_layout(char *intf_desc, uint16_t flash_sectors, uint16_t flash_page_size)
{
	...
	while (ret = sscanf(intf_desc, "/0x%x/%n", &address, &scanned),
		...
		while (ret = sscanf(intf_desc, "%d*%d%c%[^,/]%n",
			...
			if(flash_sectors > 0) sectors = flash_sectors;
			if(flash_page_size > 0) size = flash_page_size;

			printf("-> Using flash parameters: sectors %d, page size %d\n", sectors, size);
			...
			printf("Memory segment at 0x%08x %3d x %4d = "
				       ...
				       memtype & DFUSE_READABLE  ? "readable " : "",
				       memtype & DFUSE_ERASABLE  ? "eraseable " : "",
				       memtype & DFUSE_WRITEABLE ? "writeable " : "");
